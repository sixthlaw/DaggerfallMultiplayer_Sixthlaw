...
public class EntityCatcher : NetworkBehaviour
{
	...
	void CheckEntities()
	{
        ...
        {
			GameObject g = hitCollider.gameObject;
			DaggerfallEnemy daggerfallEnemy = g.GetComponent<DaggerfallEnemy>();
			if (daggerfallEnemy != null && !g.name.Contains("Quest"))
			{
				DaggerfallEntityBehaviour ed = g.GetComponent<DaggerfallEntityBehaviour>();
				
				// Check if this enemy is already synced
				if (!g.name.Contains("*synced*"))
				{
					// Try to get SetupDemoEnemy component for enemy type data
					SetupDemoEnemy setupEnemy = g.GetComponent<SetupDemoEnemy>();
					if (setupEnemy != null)
					{
						Vector3 pos = g.transform.localPosition;
				...
				enemiesFounded.Add(isRegistered(ed, g.transform.localPosition));
			}
        }
		enemies = enemiesFounded;
		enemiesDebug = enemiesFounded;
		
		// Send dungeon enemies to other players
		print("DUNGEON REQUEST SEND " + request);
		if (request != "")
		{
			cmdSendEncounters(request);
		}
